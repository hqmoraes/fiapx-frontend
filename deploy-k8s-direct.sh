...
# 4. Copiar apenas arquivos essenciais do frontend para o servidor
echo "📤 Enviando arquivos essenciais do frontend para o servidor..."

# Lista de arquivos essenciais do frontend
FRONTEND_FILES=(
    "index.html"
    "style.css"
    "config.js"
...
for file in "${FRONTEND_FILES[@]}"; do
    if [ -f "$file" ]; then
        echo "   📄 Copiando: $file"
        ${SCP_CMD} "$file" ${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_BUILD_DIR}/
    else
...
${SSH_CMD} "cd ${REMOTE_BUILD_DIR} && ls -la"
